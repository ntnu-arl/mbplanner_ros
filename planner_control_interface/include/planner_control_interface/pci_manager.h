...
class PCIManager {
 public:
  PCIManager(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
      : nh_(nh), nh_private_(nh_private), pci_status_(PCIStatus::kReady), force_stop_(false) {
    trajectory_vis_pub_ =
        nh_.advertise<visualization_msgs::MarkerArray>("pci_command_trajectory_vis", 10);
  }
  ...
  enum struct ExecutionPathType {
    ...
    kHomingPath = 1,
    kGlobalPath = 2,
    kNarrowEnvPath = 3,  // For special case, to slow down the copter in narrow env.
  ...
  virtual bool executePath(const std::vector<geometry_msgs::Pose> &path,
  ...
  // in yaw.
  // Fix the heading from the root node, relax them over time.
  virtual void allocateYawAlongPath(std::vector<geometry_msgs::Pose> &path) const = 0;
  // Fix the commanded heading to match the first segment.
  virtual void allocateYawAlongFistSegment(std::vector<geometry_msgs::Pose> &path) const = 0;
