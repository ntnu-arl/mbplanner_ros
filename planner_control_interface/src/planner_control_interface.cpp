...
PlannerControlInterface::PlannerControlInterface(ros::NodeHandle &nh,
                                                 ros::NodeHandle &nh_private,
                                                 std::shared_ptr<PCIManager> pci_manager)
    : nh_(nh), nh_private_(nh_private) {
  reference_pub_ = nh_.advertise<visualization_msgs::Marker>("ref_pose", 5);
  planner_status_pub_ = nh_.advertise<planner_msgs::PlannerStatus>("mbplanner_status", 5);

  odometry_sub_ =
      nh_.subscribe("odometry", 1, &PlannerControlInterface::odometryCallback, this);
  pose_sub_ = nh_.subscribe("pose", 1, &PlannerControlInterface::poseCallback, this);
  pci_server_ = nh_.advertiseService("planner_control_interface_trigger",
                                     &PlannerControlInterface::triggerCallback, this);
  pci_std_automatic_planning_server_ =
      nh_.advertiseService("planner_control_interface/std_srvs/automatic_planning",
                           &PlannerControlInterface::stdSrvsAutomaticPlanningCallback, this);
  pci_homing_server_ = nh_.advertiseService("pci_homing_trigger",
                                            &PlannerControlInterface::homingCallback, this);
  pci_std_homing_server_ =
      nh_.advertiseService("planner_control_interface/std_srvs/homing_trigger",
                           &PlannerControlInterface::stdSrvHomingCallback, this);
  pci_initialization_server_ = nh_.advertiseService(
      "pci_initialization_trigger", &PlannerControlInterface::initializationCallback, this);
  ...
  while (!(planner_homing_client_ = nh.serviceClient<planner_msgs::planner_homing>(
  ...
  pci_set_homing_pos_server_ = nh_.advertiseService(
      "pci_set_homing_pos", &PlannerControlInterface::setHomingPosCallback, this);
  ...
  planner_set_homing_pos_client_ =
      nh.serviceClient<planner_msgs::planner_set_homing_pos>("mbplanner/set_homing_pos");

  pci_global_server_ = nh_.advertiseService(
      "pci_global", &PlannerControlInterface::globalPlannerCallback, this);
  planner_global_client_ = nh.serviceClient<planner_msgs::planner_global>("mbplanner/global");
  pci_stop_server_ =
      nh_.advertiseService("pci_stop", &PlannerControlInterface::stopPlannerCallback, this);
  pci_std_stop_server_ =
      nh_.advertiseService("planner_control_interface/std_srvs/stop",
                           &PlannerControlInterface::stdSrvStopPlannerCallback, this);

  planner_geofence_client_ =
      nh.serviceClient<planner_msgs::planner_geofence>("mbplanner/geofence");
  ...
  pci_std_global_last_specified_frontier_server_ = nh_.advertiseService(
      ...
      &PlannerControlInterface::stdSrvReplanLastSpecifiedFrontierCallback, this);
...
bool PlannerControlInterface::globalPlannerCallback(planner_msgs::pci_global::Request &req,
...
bool PlannerControlInterface::stopPlannerCallback(planner_msgs::pci_stop::Request &req,
...
bool PlannerControlInterface::stdSrvStopPlannerCallback(std_srvs::Trigger::Request &req,
...
bool PlannerControlInterface::addGeofenceCallback(planner_msgs::pci_geofence::Request &req,
                                                  planner_msgs::pci_geofence::Response &res) {
  planner_msgs::planner_geofence plan_srv;
  plan_srv.request.rectangles = req.rectangles;
  if (planner_geofence_client_.call(plan_srv)) res.success = plan_srv.response.success;
...
bool PlannerControlInterface::stdSrvSetHomingPositionHereCallback(
  ...
  res.success = setHomingPosCallback(set_homing_pos_request, set_homing_pos_response);
...
bool PlannerControlInterface::homingCallback(planner_msgs::pci_homing_trigger::Request &req,
...
bool PlannerControlInterface::stdSrvHomingCallback(std_srvs::Trigger::Request &req,
                                                   std_srvs::Trigger::Response &res) {
...
bool PlannerControlInterface::triggerCallback(planner_msgs::pci_trigger::Request &req,
  ...
  if (pci_manager_->getStatus() == PCIManager::PCIStatus::kError) {
    ...
    if ((!req.set_auto) && (trigger_mode_ == PlannerTriggerModeType::kAuto)) {
      ...
      trigger_mode_ = PlannerTriggerModeType::kManual;
    } else if ((req.set_auto) && (trigger_mode_ == PlannerTriggerModeType::kManual)) {
  ...
  while (cont) {
    ...
    if (pci_status == PCIManager::PCIStatus::kReady) {
      ...
      // Priority 2: Check if require initialization step.
        ...
        run_en_ = false;
        ROS_INFO_STREAM("PlannerControlInterface: Running Planner ("
  ...
  for (int ind = 0; ind < kBBoxLevel; ++ind) {
    ...
    if (planner_client_.call(plan_srv)) {
      if (!plan_srv.response.path.empty()) {
        // Execute path.
        if (exe_path) {
          if ((!force_forward_) || (plan_srv.response.status != plan_srv.response.kBackward) ||
              (ind == (kBBoxLevel - 1))) {
            if (ind == (kBBoxLevel - 1))
              ROS_WARN(
...
void PlannerControlInterface::publishPlannerStatus(
  ...
  planner_status_msg.trigger_mode.mode =
      static_cast<decltype(planner_status_msg.trigger_mode.mode)>(trigger_mode_);
...
void PlannerControlInterface::poseCallback(
  ...
  current_pose_.position.x = pose.pose.pose.position.x;
  current_pose_.position.y = pose.pose.pose.position.y;
  current_pose_.position.z = pose.pose.pose.position.z;
  current_pose_.orientation.x = pose.pose.pose.orientation.x;
  current_pose_.orientation.y = pose.pose.pose.orientation.y;
  current_pose_.orientation.z = pose.pose.pose.orientation.z;
  current_pose_.orientation.w = pose.pose.pose.orientation.w;
  pci_manager_->setState(current_pose_);
  pose_is_ready_ = true;
}
