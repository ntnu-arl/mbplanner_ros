...
class RandomSamplerBase {
 public:
  ...
  void setParams(int ind, BoundedSpaceParams& global_params, BoundedSpaceParams& local_params);
...
class RandomSampler {
 public:
  ...
  ~RandomSampler();
  bool loadParams(std::string ns);
  bool setParams(BoundedSpaceParams& global_params, BoundedSpaceParams& local_params);
  ...
  double getZOffset() {
    const int kZIndex = 2;
    if (random_sampler_base_.size() >= 3) return random_sampler_base_[kZIndex].getZOffset();
  }
