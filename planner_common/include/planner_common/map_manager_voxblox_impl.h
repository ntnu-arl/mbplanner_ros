  ...
  for (FloatingPoint x = -halfrange_in_voxels[0]; x <= halfrange_in_voxels[0]; x++) {
    for (FloatingPoint y = -halfrange_in_voxels[1]; y <= halfrange_in_voxels[1]; y++) {
      for (FloatingPoint z = -halfrange_in_voxels[2]; z <= halfrange_in_voxels[2]; z++) {
        ...
        getBlockAndVoxelIndexFromGlobalVoxelIndex(voxel_offset_index + center_index,
...
class MapManagerVoxblox : MapManager {
 public:
  ...
  VoxelStatus getBoxStatus(const Eigen::Vector3d& center, const Eigen::Vector3d& size,
                           bool stop_at_unknown_voxel) const;
  float getVoxelDistance(const Eigen::Vector3d& center);
  VoxelStatus getPathStatus(const Eigen::Vector3d& start, const Eigen::Vector3d& end,
                            const Eigen::Vector3d& box_size, bool stop_at_unknown_voxel) const;
  bool augmentFreeBox(const Eigen::Vector3d& position, const Eigen::Vector3d& box_size);
  void getScanStatus(Eigen::Vector3d& pos, std::vector<Eigen::Vector3d>& multiray_endpoints,
                     std::tuple<int, int, int>& gain_log,
                     std::vector<std::pair<Eigen::Vector3d, VoxelStatus>>& voxel_log);
  // void setSurfaceColor(Eigen::Vector3d& pos, std::vector<Eigen::Vector3d>&
  // multiray_endpoints);
  ...
  void extractLocalMap(const Eigen::Vector3d& center, const Eigen::Vector3d& bounding_box_size,
                       std::vector<Eigen::Vector3d>& occupied_voxels,
                       std::vector<Eigen::Vector3d>& free_voxels);

  void extractLocalMapAlongAxis(const Eigen::Vector3d& center, const Eigen::Vector3d& axis,
                                const Eigen::Vector3d& bounding_box_size,
                                std::vector<Eigen::Vector3d>& occupied_voxels,
                                std::vector<Eigen::Vector3d>& free_voxels);
  ...
  void getNumVoxels(int& num_occupied_voxels, int& num_free_voxels) const;

 private:
  VoxelStatus getBoxStatusInVoxels(const voxblox::LongIndex& bounding_box_center,
