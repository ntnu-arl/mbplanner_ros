...
class Trajectory {
 public:
  ...
  static bool compareTwoTrajectories(const PathType &path_1, const PathType &path_2,
                                     double dist_threshold = 1.0, double discrete_length = 0.2,
                                     bool shorten_to_same_length = true,
                                     bool scale_with_length = true);
  // In this case we prefer not scale with path length to amplify difference
  // between long paths.
  static double computeDistanceBetweenTrajectoryAndDirection(const PathType &path,
                                                             double heading,
                                                             double discrete_length = 0.2,
                                                             bool scale_with_length = false);

  //
  static double computeDistanceBetweenTwoTrajectories(const PathType &path_1,
  ...
  //
  static void extractPathFromTrajectory(const TrajectoryType &traj, PathType &path);
