...
bool RandomSampler::setBound(Eigen::Vector3d& min_val, Eigen::Vector3d& max_val) {
...
void RandomSampler::pushSample(StateVec& state, bool valid) {
  if (valid) {
    if (valid_samples_.size() < kValidSamplesMaxSize) valid_samples_.emplace_back(state);
  } else {
    if (invalid_samples_.size() < kInvalidSamplesMaxSize) invalid_samples_.emplace_back(state);
  }
}
