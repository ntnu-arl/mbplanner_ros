...
double MapManagerOctomap::getResolution() const { return octomap_manager_->getResolution(); }
...
MapManagerOctomap::VoxelStatus MapManagerOctomap::getRayStatus(
  ...
  volumetric_mapping::OctomapManager::CellStatus status =
      octomap_manager_->getVisibility(view_point, voxel_to_test, stop_at_unknown_voxel);
  return convertStatus(status);
}
...
MapManagerOctomap::VoxelStatus MapManagerOctomap::getPathStatus(
    ...
    bool stop_at_unknown_voxel) const {
  // volumetric_mapping::OctomapManager::CellStatus status =
  //     octomap_manager_->getLineStatusBoundingBox(start, end, box_size);
  volumetric_mapping::OctomapManager::CellStatus status =
...
bool MapManagerOctomap::augmentFreeBox(const Eigen::Vector3d& position,
  ...
  octomap_manager_->setFree(position, box_size, volumetric_mapping::BoundHandling::kDefault);
  return true;
}
...
void MapManagerOctomap::getScanStatus(
  ...
  std::vector<std::pair<Eigen::Vector3d, volumetric_mapping::OctomapManager::CellStatus>>
      voxel_log_temp;
...
void MapManagerOctomap::getFreeSpacePointCloud(std::vector<Eigen::Vector3d> multiray_endpoints,
                                               StateVec state,
...
void MapManagerOctomap::extractLocalMap(const Eigen::Vector3d& center,
...
void MapManagerOctomap::extractLocalMapAlongAxis(const Eigen::Vector3d& center,
