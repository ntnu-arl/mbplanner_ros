...
// ESDF
template <>
bool MapManagerVoxblox<voxblox::EsdfServer, voxblox::EsdfVoxel>::checkUnknownStatus(
    const voxblox::EsdfVoxel* voxel) const {
...
template <typename SDFServerType, typename SDFVoxelType>
typename MapManagerVoxblox<SDFServerType, SDFVoxelType>::VoxelStatus MapManagerVoxblox<
    SDFServerType, SDFVoxelType>::getVoxelStatus(const Eigen::Vector3d& position) const {
  SDFVoxelType* voxel =
      sdf_layer_->getVoxelPtrByCoordinates(position.cast<voxblox::FloatingPoint>());
...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::getBoxStatusInVoxels(
  ...
  for (voxel_index.x() = box_center.x() - box_voxels.x() / 2;
       voxel_index.x() <= box_center.x() + box_voxels.x() / 2; voxel_index.x()++) {
    for (voxel_index.y() = box_center.y() - box_voxels.y() / 2;
         voxel_index.y() <= box_center.y() + box_voxels.y() / 2; voxel_index.y()++) {
      for (voxel_index.z() = box_center.z() - box_voxels.z() / 2;
           voxel_index.z() <= box_center.z() + box_voxels.z() / 2; voxel_index.z()++) {
...
float MapManagerVoxblox<SDFServerType, SDFVoxelType>::getVoxelDistance(
  ...
  float voxel_size_inv =
      1.0 / voxel_size;  // Get the center of the bounding box as a global index.
  voxblox::LongIndex center_voxel_index = voxblox::getGridIndexFromPoint<voxblox::LongIndex>(
      center.cast<voxblox::FloatingPoint>(), voxel_size_inv);
...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::getPathStatus(
  ...
  const voxblox::Point start_scaled = start.cast<voxblox::FloatingPoint>() * voxel_size_inv;
...
template <typename SDFServerType, typename SDFVoxelType>
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::getNumVoxels(int& num_occupied_voxels,
                                                                  int& num_free_voxels) const {
  pcl::PointCloud<pcl::PointXYZI> pointcloud;
  num_occupied_voxels = 0;
  num_free_voxels = 0;
// if(using_tsdf)
#if using_tsdf
  createDistancePointcloudFromTsdfLayer(*sdf_layer_, &pointcloud);
#else
  createDistancePointcloudFromEsdfLayer(*sdf_layer_, &pointcloud);
#endif
  const float distance_thres =
      occupancy_distance_voxelsize_factor_ * sdf_layer_->voxel_size() + 1e-6;
  for (auto& p : pointcloud.points) {
    double distance = p.intensity;
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::getScanStatus(
  ...
  const voxblox::Point start_scaled = pos.cast<voxblox::FloatingPoint>() * voxel_size_inv;
  ...
  // a set to avoid double-counting
  // Important: do not use <VoxelIndex> type directly, will break voxblox's
  // implementations
  ...
  raycast_free_vec_;
  raycast_unknown_vec_.reserve(multiray_endpoints.size() *
  tsdf_integrator_config_.max_ray_length_m * voxel_size_inv); //optimize for
  number of rays
  raycast_free_vec_.reserve(multiray_endpoints.size() *
  tsdf_integrator_config_.max_ray_length_m * voxel_size_inv); //optimize for
  number of rays
  raycast_occupied_vec_.reserve(multiray_endpoints.size()); //optimize for
  number of rays*/
  voxel_log.reserve(multiray_endpoints.size() * tsdf_integrator_config_.max_ray_length_m *
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::extractLocalMap(
    ...
    std::vector<Eigen::Vector3d>& occupied_voxels, std::vector<Eigen::Vector3d>& free_voxels) {
  ...
  origin_offset = center_corrected - Eigen::Vector3d(Nx * resolution / 2, Ny * resolution / 2,
                                                     Nz * resolution / 2);
  for (double x_ind = 0; x_ind <= Nx; ++x_ind) {
    for (double y_ind = 0; y_ind <= Ny; ++y_ind) {
      for (double z_ind = 0; z_ind <= Nz; ++z_ind) {
        Eigen::Vector3d pos(x_ind * resolution, y_ind * resolution, z_ind * resolution);
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::extractLocalMapAlongAxis(
    ...
    const Eigen::Vector3d& bounding_box_size, std::vector<Eigen::Vector3d>& occupied_voxels,
