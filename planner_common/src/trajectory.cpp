...
bool Trajectory::compareTwoTrajectories(const TrajectoryType &traj_1,
                                        const TrajectoryType &traj_2, double dist_threshold,
                                        double discrete_length, bool shorten_to_same_length,
                                        bool scale_with_length) {
  if ((traj_1.size() <= 0) || (traj_2.size() <= 0) || (discrete_length <= 0)) return false;

  PathType path_1;
  extractPathFromTrajectory(traj_1, path_1);
  PathType path_2;
  extractPathFromTrajectory(traj_2, path_2);
  return (compareTwoTrajectories(path_1, path_2, dist_threshold, discrete_length,
                                 shorten_to_same_length, scale_with_length));
}

bool Trajectory::compareTwoTrajectories(const PathType &path_1, const PathType &path_2,
                                        double dist_threshold, double discrete_length,
                                        bool shorten_to_same_length, bool scale_with_length) {
  double dist = computeDistanceBetweenTwoTrajectories(
      path_1, path_2, discrete_length, shorten_to_same_length, scale_with_length);
...
double Trajectory::computeDistanceBetweenTwoTrajectories(const PathType &path_1,
  ...
  if ((path_1.size() <= 0) || (path_2.size() <= 0) || (discrete_length <= 0)) return dist_ret;
...
double Trajectory::computeDistanceBetweenTrajectoryAndDirection(const PathType &path,
                                                                double heading,
                                                                double discrete_length,
                                                                bool scale_with_length) {
  if ((path.size() <= 0) || (discrete_length <= 0)) return std::numeric_limits<double>::max();
  ...
  Eigen::Vector3d uvector = Eigen::AngleAxisd(heading, Eigen::Vector3d::UnitZ()) *
...
void Trajectory::extractPathFromTrajectory(const TrajectoryType &traj, PathType &path) {
  path.clear();
  for (auto t = traj.begin(); t != traj.end(); ++t) {
    path.push_back(VectorType((*t).position.x, (*t).position.y, (*t).position.z));
  }
}
...
double Trajectory::computeDTWDistance(const PathType &pa, const PathType &pb) {
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 1; j <= m; ++j) {
      ...
      dist[i][j] = d + std::min(std::min(dist[i - 1][j - 1], dist[i - 1][j]), dist[i][j - 1]);
    }
  }
