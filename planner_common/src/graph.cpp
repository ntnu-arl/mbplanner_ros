...
Graph::EdgeDescriptorPair Graph::addEdge(int u_id, int v_id, double weight) {
  ...
  return boost::add_edge(vertex_descriptors_[u_id], vertex_descriptors_[v_id], weight, graph_);
}
...
void Graph::removeEdge(int u_id, int v_id) {
  ...
  boost::remove_edge(vertex_descriptors_[u_id], vertex_descriptors_[v_id], graph_);
}
...
bool Graph::findDijkstraShortestPaths(VertexDescriptor &source,
                                      std::vector<VertexDescriptor> &shortest_paths,
                                      std::vector<double> &shortest_distances) {
  ...
  boost::dijkstra_shortest_paths(
      ...
      boost::predecessor_map(boost::make_iterator_property_map(
                                 shortest_paths.begin(), get(boost::vertex_index, graph_)))
          .distance_map(boost::make_iterator_property_map(shortest_distances.begin(),
...
std::tuple<int, int, double> Graph::getEdgeProperty(GraphType::edge_iterator e) {
  return std::make_tuple(getVertexID(boost::source(*e, graph_)),
