...
bool SensorParamsBase::loadParams(std::string ns) {
  ...
  if (type == SensorType::kCamera) {
    ...
    ROS_INFO("Computed multiray_endpoints for volumetric gain: [%d] points.",
             frustum_endpoints_B.size());
  }

  // Compute a number to compare to check for frontier.
  num_voxels_full_fov = (fov[0] / resolution[0]) * (fov[1] / resolution[1]) * max_range;

  ROSPARAM_INFO("Done.");
  return true;
}

void SensorParamsBase::getFrustumEndpoints(StateVec &state, std::vector<Eigen::Vector3d> &ep) {
...
void SensorParamsBase::getFrustumEdges(StateVec &state, std::vector<Eigen::Vector3d> &edges) {
  Eigen::Vector3d origin(state[0], state[1], state[2]);
  Eigen::Matrix3d rot_W2B;
  rot_W2B = Eigen::AngleAxisd(state[3], Eigen::Vector3d::UnitZ()) *
            Eigen::AngleAxisd(0.0, Eigen::Vector3d::UnitY()) *
...
bool SensorParamsBase::isInsideFOV(StateVec &state, Eigen::Vector3d &pos) {
  ...
  Eigen::Vector3d pos_S =
      rot_S2B * Eigen::AngleAxisd(-state[3], Eigen::Vector3d::UnitZ()) * (pos - origin) -
      center_offset;
  ...
  if (type == SensorType::kCamera) {
    ...
    if ((std::abs(h_angle) > (fov[0] / 2)) || (std::abs(v_angle) > (fov[1] / 2))) return false;
  } else {
    // Unsupported sensor.
    return false;
  }
...
bool SensorParamsBase::isFrontier(double num_unknown_voxels_normalized) {
  double unknown_percentage = num_unknown_voxels_normalized / num_voxels_full_fov;
...
bool RobotParams::loadParams(std::string ns) {
  ...
  if ((!ros::param::get(param_name, param_val)) || (param_val.size() != 3) ||
      (param_val[0] < size_extension_min[0]) || (param_val[1] < size_extension_min[1]) ||
      (param_val[2] < size_extension_min[2])) {
    param_val.resize(3);
    param_val[0] = size_extension_min[0];
    param_val[1] = size_extension_min[1];
    param_val[2] = size_extension_min[2];
    ROSPARAM_WARN(param_name, "{" << param_val[0] << "," << param_val[1] << "," << param_val[2]
                                  << "}");
  }
...
void RobotParams::getPlanningSize(Eigen::Vector3d &psize) {
  ...
  switch (bound_mode) {
    case BoundModeType::kExtendedBound:
      ...
      break;
    case BoundModeType::kRelaxedBound:
      psize = size + relax_ratio * size_extension_min + (1 - relax_ratio) * size_extension;
      break;
    case BoundModeType::kMinBound:
...
bool PlanningParams::loadParams(std::string ns) {
  ...
  param_name = ns + "/overlapped_frontier_radius";
  if (!ros::param::get(param_name, overlapped_frontier_radius)) {
    overlapped_frontier_radius = 2.5;
    ROSPARAM_WARN(param_name, 2.5);
  }

  param_name = ns + "/sparse_radius";
  if (!ros::param::get(param_name, sparse_radius)) {
    sparse_radius = 1.5;
    ROSPARAM_WARN(param_name, 1.5);
  }
...
bool MBParams::loadParams(std::string ns) {
  ros::param::get(ns + "/ax_min", ax_min);
  ros::param::get(ns + "/ay_min", ay_min);
  ros::param::get(ns + "/az_min", az_min);
  ros::param::get(ns + "/ax_max", ax_max);
  ros::param::get(ns + "/ay_max", ay_max);
  ros::param::get(ns + "/az_max", az_max);
  ros::param::get(ns + "/v_max", v_max);
  ros::param::get(ns + "/t", t);
  ros::param::get(ns + "/dt", dt);
  ros::param::get(ns + "/iterations", iterations);
  ros::param::get(ns + "/sample_steps", sample_steps);
  ros::param::get(ns + "/max_sampling_iters", max_sampling_iters);
  ros::param::get(ns + "/var_no_of_sampling", var_no_of_sampling);
  ros::param::get(ns + "/later_iter_samples", later_iter_samples);
  ros::param::get(ns + "/kdtree_range", kdtree_range);
  ros::param::get(ns + "/safety_distance", safety_distance);
  ros::param::get(ns + "/min_vertex_separation", min_vertex_separation);
  ros::param::get(ns + "/obst_steps", obst_steps);
  ros::param::get(ns + "/branch_limit", branch_limit);
  ros::param::get(ns + "/use_current_state", use_current_state);
  ros::param::get(ns + "/extrapolation_dist", extrapolation_dist);
  ...
  ros::param::get(ns + "/a_cone", a_cone);
  ros::param::get(ns + "/use_angles", use_angles);
  ros::param::get(ns + "/clustering", clustering);
  ros::param::get(ns + "/max_traverse_length", max_traverse_length);
  ros::param::get(ns + "/min_edge_length", min_edge_length);
  ros::param::get(ns + "/path_direction_penalty", path_direction_penalty);
  ros::param::get(ns + "/path_length_penalty", path_length_penalty);
  ros::param::get(ns + "/curvature_penalty", curvature_penalty);
  ros::param::get(ns + "/safety_gain", safety_gain);
  ros::param::get(ns + "/free_voxel_gain", free_voxel_gain);
  ros::param::get(ns + "/unknown_voxel_gain", unknown_voxel_gain);
  ros::param::get(ns + "/occupied_voxel_gain", occupied_voxel_gain);
  ros::param::get(ns + "/safety_aware_enable", safety_aware_enable);
  ros::param::get(ns + "/similarity_dist", similarity_dist);
  std::vector<std::string> parse_str_list;
  ros::param::get(ns + "/exp_sensor_list", exp_sensor_list);
  if (exp_sensor_list.size() <= 0) {
    ROSPARAM_ERROR(ns + "/exp_sensor_list");
  } else {
    std::string str_tmp = "Sensors for exploration: ";
    for (int i = 0; i < exp_sensor_list.size(); ++i) {
      str_tmp += exp_sensor_list[i] + ", ";
